...
class Joint extends THREE.Group {
  constructor(parentJoint, pos, params, shape) {
    ...
    else this.image = new THREE.Group();
  ...
  detach(image) {
    if (
      this.imageWrapper.children.includes(
...
class Finger extends Phalange {
  constructor(parentJoint, leftOrRight, number) {
    ...
    this.minRot = new THREE.Vector3(
      Math.min(minX, maxX),
      Math.min(this.y, 2 * this.y),
      thumb ? -90 : -10
    );
    this.maxRot = new THREE.Vector3(
      Math.max(minX, maxX),
      Math.max(this.y, 2 * this.y),
      thumb ? 45 : 120
    );
...
Mannequin.blend = function (posture0, posture1, k) {
  ...
  function lerp(data0, data1, k) {
    if (data0 instanceof Array) {
      var result = [];
      for (var i in data0) result.push(lerp(data0[i], data1[i], k));
      return result;
    } else {
...
Mannequin.convert6to7 = function (posture) {
  ...
  // 0..12
  for (var i = 0; i <= 12; i++) data.push(posture.data[i]);

  // 13
  var a = posture.data[13][0],
    b = posture.data[13][1];

  for (var i = 0; i < 5; i++) data.push([0, a, 0, b / 2, 0, b / 2]);

  // 14..16
  for (var i = 14; i <= 16; i++) data.push(posture.data[i]);

  // 17
  (a = posture.data[17][0]), (b = posture.data[17][1]);
